<?php
namespace CpCompanion\Modules\Ytvideosl\Widgets;
...
class Ytvideosl extends Widget_Base {
	...
	public function get_title() {
		return  esc_html__( 'YouTube Video Slider', 'cp-companion' );
	}

	public function get_icon() {
		return 'cp-ea-icons cp-ytvideosl';
	}
    ...
    protected function _register_controls() {
        ...
        $this->start_controls_section(
            'cp_companion_ytvideosl_section',
            [
                'label' => __( 'YouTube Videos', 'cp-companion' ),
                'type' => \Elementor\Controls_Manager::SECTION,
            ]
        );

        $this->add_control(
            'cp_companion_ytvideosl_channel',
            [
                'label' => __( 'YouTube Channel ID', 'cp-companion' ),
        ...
        $this->add_control(
            'cp_companion_ytvideosl_key',
            [
                'label' => __( 'YouTube API key', 'cp-companion' ),
                'type' => \Elementor\Controls_Manager::TEXT,
        ...
        $this->add_control(
            'cp_companion_ytvideosl_number',
            [
                'label' => __( 'Total Number of Videos', 'cp-companion' ),
        ...
        $this->add_control(
            'cp_companion_ytvideosl_sort',
            [
                'label' => __( 'Sort By', 'cp-companion' ),
                'type' => \Elementor\Controls_Manager::SELECT,
                'default' => 'date',
                'options' => [
                    'date' => __( 'Date', 'cp-companion' ),
                    'rating' => __( 'Rating', 'cp-companion' ),
                    'title' => __( 'Title - Alphabetically', 'cp-companion' ),
                    'viewCount' => __( 'View Count', 'cp-companion' ),
                ],
                'frontend_available' => true,
            ]
        );
        ...
        $this->add_control(
            'cp_companion_ytvideosl_lay',
            [
                'label' => __( 'Layout', 'cp-companion' ),
                'type' => \Elementor\Controls_Manager::SELECT,
                'default' => 'scroller',
                'options' => [
                    'slider' => __( 'Slider', 'cp-companion' ),
                ],
                'frontend_available' => true,
            ]
        );
        ...
        */
        ...
        $this->add_control('vt_color',
            [
                'label'     => esc_html__( 'Video Title Color', 'cp-companion' ),
    ...
    protected function render() {
        ...
        $cp_companion_ytvideosl_lay = (!empty($settings['cp_companion_ytvideosl_lay']))?$settings['cp_companion_ytvideosl_lay']:"scroller";
        
        $this->add_render_attribute( 'cp-companion-attr', 'class', 'cp-companion-ytvideosl' );
        $this->add_render_attribute( 'cp-companion-attr', 'class', 'lay-'.$cp_companion_ytvideosl_lay );
        ?>
        <div <?php echo $this->get_render_attribute_string('cp-companion-attr')?> >
            ...
            $channel_id = $settings['cp_companion_ytvideosl_channel'];
            $key = $settings['cp_companion_ytvideosl_key'];
            $maxVids = $settings['cp_companion_ytvideosl_number'];
            $order = $settings['cp_companion_ytvideosl_sort'];
            ...
            <div class="cp-companion-yt-player">

                <div class="cp-companion-video-holder clearfix"> 

                    <div class="big-video">
                        <div class="big-video-inner">
                            <i class="dashicons dashicons-no-alt"></i>
                            <div id="cp-companion-video-placeholder"></div>
                        </div>
                        <div class="video-controls">
                            ...
                            <div class="video-track">
                                <div class="video-current-playlist section-title">
                                    <?php esc_html_e( 'Fetching Video Title..', 'cp-companion' ) ?>
                                </div>
                                <div class="video-duration-time section-desc wow fadeInRight">
                                    <span class="video-current-time">0:00</span>
                                    /
                                    <span class="video-duration"><?php esc_html_e( 'Loading..', 'cp-companion' ) ?></span>     
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="video-thumbs">
                        <div class="cp-companion-video-thumbnails">
                            <?php
                            ...
                            foreach ( $video_list as $video ) {
                                ...
                                if ( is_array( $video_api_array ) && !empty( $video_api_array[ 'items' ] ) ) {
                                    ...
                                    ?>
                                    <div class="cp-companion-video-list" data-index="<?php echo absint($i); ?>" data-video-id="<?php echo esc_attr($video); ?>"> 
                                    ...
                                    ?>  
                                    <div class="cp-companion-video-list clearfix">  
                                        <div class="video-title-duration">
                                            <h4><i><?php _e( 'Either this video has been removed or you don\'t have access to watch this video', 'cp-companion' ); ?></i></h4>
                                        </div>
                                    </div>
                                    <?php
                                }
            ...
            <script type="text/javascript">
                ...
                function onYouTubeIframeAPIReady() {
                    player = new YT.Player('cp-companion-video-placeholder', {
                ...
                function initialize() {
                    ...
                    jQuery('.video-current-playlist').text(jQuery('.cp-companion-video-list:first').text());
                    jQuery('.cp-companion-video-list:first').addClass('video-active');
                ...
                function updateButtonStatus(playerStatus) {
                    ...
                    if (playerStatus == -1) {
                        ...
                        var currentElement = jQuery('.cp-companion-video-list').map(function () {
                ...
                jQuery(function ($) {
                    ...
                    $('body').on('click','.cp-companion-video-list', function () {
...
*
*/
if ( !function_exists( 'cp_companion_pro_youtube_duration' ) ) {
    function cp_companion_pro_youtube_duration( $duration ) {
